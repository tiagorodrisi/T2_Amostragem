...
Leads_Zap$data_criado_em <- as.Date(Leads_Zap$data_criado_em, format="%d-%m-%Y")

# Agrupando os dados pelo código do imóvel e calcular as estatísticas necessárias
dados_agrupados <- Leads_Zap %>%
...
dados_agrupados$data_min <- as.Date(dados_agrupados$data_min, format="%d-%m-%Y")
dados_agrupados$data_max <- as.Date(dados_agrupados$data_max, format="%d-%m-%Y")
...
# Calculando a margem de erro para um nível de confiança de 90%
nivel_conf <- 0.90
...
# Verificar se a média das 3 amostras estão dentro do intervalo de confiança
